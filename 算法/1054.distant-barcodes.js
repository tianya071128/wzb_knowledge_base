...
class MaxHelp {
  ...
  #siftUp(i) {
    ...
    while (p >= 0 && this.#maxHeap[p][1] < this.#maxHeap[i][1]) {
      // 交换位置
      this.#swap(p, i);
  ...
  #siftDown(i) {
    while (true) {
      ...
      if (l < this.size() && this.#maxHeap[l][1] > this.#maxHeap[target][1])
        target = l;
      if (r < this.size() && this.#maxHeap[r][1] > this.#maxHeap[target][1])
        target = r;

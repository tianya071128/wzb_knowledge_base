...
module.exports = {
  /**
   * 暴露的自定义规则：使用这些规则，使用不带前缀的插件名，后跟一个 /，然后是规则名。e.g：vue/array-bracket-newline
   * 
   * 注意的是：必须在 plugins 配置项中引用这个插件才能使用这些规则。
   *          在下面的 configs 共享配置项中，会配置 plugins: ['vue']，所以可以使用这些规则
   */
  rules: {
  ...
  /**
   * 暴露的配置项：在默认情况下，配置不会启用插件中的任何规则，而且应该作为一个独立的配置来处理。
   *              这意味着，你必须在 plugins 数组中指定插件名和任何你想使用的插件中的规则。
   *              也就是说插件不会默认使用这些规则或配置，只有用户在配置文件中显式定义配置
   * 在用户配置文件使用 e.g：
  ...
  /**
   * 暴露一个处理器：告诉 ESLint 如何处理 JS 之外的文件 -- https://eslint.org/docs/developer-guide/working-with-plugins#processors-in-plugins
   */
  processors: {
    /**
     * 如果处理器名称以 . 开头，ESLint 会自动将处理器应用于文件类型。
     * 否则的话，就需要在配置文件中配置 processor 选项：e.g：a-plugin/markdown
     */
    '.vue': require('./processor')
  },
  /**
   * 暴露额外的环境以在 ESLint 中使用
   * 在 ESLint 中使用这个环境，你可以使用不带前缀的插件名，后跟一个 /，然后是环境名
   * e.g："env": "vue/setup-compiler-macros"
   */
  environments: {
    'setup-compiler-macros': {
      // 同配置文件中的 globals 一样。key 是全局变量的名字，值为 true允许全局变量被覆盖，false 不允许覆盖
      globals: {

...
module.exports = {
  ...
  *iterateArrowFunctionExpression(node, groupName) {
  ...
  /**
   * Check whether the templateBody of the program has invalid EOF or not. 检查程序的模板体是否有无效的 EOF
   * @param {Program} node The program node to check. 对程序节点进行检查
   * @returns {boolean} `true` if it has invalid EOF.
   */
  hasInvalidEOF(node) {
    const body = node.templateBody // vue 文件中 template 中 AST
...
/**
 * Register the given visitor to parser services. 将给定的访问者注册到解析器服务
 * If the parser service of `vue-eslint-parser` was not found, 如果没有找到' vue-eslint-parser '的解析器服务
 * this generates a warning. 这将产生一个警告
 *
 * @param {RuleContext} context The rule context to use parser services.
 * @param {TemplateListener} templateBodyVisitor The visitor to traverse the template body.
 * @param {RuleListener} [scriptVisitor] The visitor to traverse the script.
...
function defineTemplateBodyVisitor(
  ...
  if (context.parserServices.defineTemplateBodyVisitor == null) {
    const filename = context.getFilename() // 返回与源文件关联的文件名。
    if (path.extname(filename) === '.vue') {
      // 用来发布警告或错误（取决于你所使用的配置）
      context.report({
        loc: { line: 1, column: 0 }, // 问题位置的对象
        // 有问题的消息
        message:
          // 使用最新的 vue-eslint-parser
          'Use the latest vue-eslint-parser. See also https://eslint.vuejs.org/user-guide/#what-is-the-use-the-latest-vue-eslint-parser-error.'
      })
    }
    return {} // 返回空对象，此时这个规则不进行检测
  }

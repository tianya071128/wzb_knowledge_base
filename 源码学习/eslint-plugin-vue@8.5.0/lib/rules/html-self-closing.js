...
module.exports = {
  ...
  meta: {
    // 规则的类型
    type: 'layout', // layout：意味着规则主要关心空格、分号、逗号和括号，以及程序中决定代码外观而不是执行方式的所有部分。这些规则适用于AST中没有指定的代码部分。
    docs: {
  ...
  create(context) {
    const sourceCode = context.getSourceCode() // 检查内容源码
    /**
     * 规则配置项：
     *  e.g："vue/html-self-closing": ["error", {
     *         "html": {
     *           "void": "never",
     *           "normal": "always",
     *           "component": "always"
    ...
    /**
     * 返回一个对象，用于在 ESLint 遍历 AST 时，用来访问节点的方法 -- https://eslint.bootcss.com/docs/developer-guide/working-with-rules#rule-basics
     */
    return utils.defineTemplateBodyVisitor(
      context,
      {
        // VElement：template 中标签(或组件)类别
        VElement(node) {
          if (hasInvalidEOF) {
            return
          }
      ...
      {
        // 注册监听的类别，Program 是 AST 最顶层的类别
        Program(node) {
          // node：注册 AST 节点类型的 AST，应该是用来检测一些内容

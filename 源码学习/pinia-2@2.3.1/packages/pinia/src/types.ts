...
> extends StoreProperties<Id> {
  ...
  /**
   * Vue 2 only. Is the store ready. Used for store cross usage. Getters automatically compute when they are added to 仅限Vue 2。商店准备好了吗。用于商店交叉使用。Getters在添加到时会自动计算
   * the store, before the store is actually ready, this allows to avoid calling the computed function yet. 在存储真正准备就绪之前，存储可以避免调用计算函数。
   *
   * @internal
   */
  _r?: boolean

...
1. **显式指定源引用**: 通过[引用规范]([引用规范](./refspec.html#git-push-中的引用规范-推送本地-→-远程)) `<src>:<dst>` 明确指定本地源引用
...
| :----------------------------- | :-------------- | :-------------- |
| `git push`                     | 当前分支        | 上游分支        |
| `git push origin main`         | 本地main分支    | 远程main分支    |
| `git push origin feature:main` | 本地feature分支 | 远程main分支    |
| `git push origin HEAD:feature` | 当前提交        | 远程feature分支 |

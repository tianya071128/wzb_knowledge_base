...
差分数组是一种常用于处理频繁区间更新（如对原数组的某个连续子区间统一加上一个值）和最终单点查询问题的数据结构。

通过记录相邻元素的差值，将区间操作转化为对差分数组首尾两个端点的修改，从而将每次操作的复杂度从 O(n) 降至 O(1)，最后通过一次前缀和还原出最终数组。

### 构造等分数组

给定一个原数组 `a`，长度为 `n`（索引从 0 开始），其差分数组 `diff` 定义为：

- `diff[0] = a[0]`
- 对于 `i = 1, 2, ..., n-1`，`diff[i] = a[i] - a[i-1]`

也可以为了便于处理区间端点的边界，将 `diff` 长度设为 `n+1`，并令 `diff[n] = 0`（或 `-a[n-1]`），这样在修改区间 `[l, r]` 时，`r+1` 不会越界。

### 核心性质

- **区间修改**：若要将原数组 `a` 的区间 `[l, r]` 内所有元素加上 `k`，只需：

  - `diff[l] += k`
  - 如果 `r+1 < n`，则 `diff[r+1] -= k`（若使用长度 n+1 的 diff，则直接对 `diff[r+1]` 操作，不必判断边界）

- **还原原数组**：对差分数组求前缀和即可得到原数组。

  ```text
  a[0] = diff[0]
  a[i] = diff[0] + diff[1] + ... + diff[i]   (i ≥ 1)
  ```

### 原理说明

为什么这样能实现区间修改？考虑差分数组的定义：`diff[i] = a[i] - a[i-1]`。

- 在区间 `[l, r]` 每个元素加 `k` 后，`a[l]` 比 `a[l-1]` 多增加了 `k`，所以 `diff[l]` 要增加 `k`。
- 对于 `i = l+1 ... r`，`a[i]` 和 `a[i-1]` 同时增加了 `k`，它们的差不变，因此 `diff[i]` 不变。
- 对于 `r+1`，`a[r+1]` 没有增加，而 `a[r]` 增加了 `k`，所以 `a[r+1] - a[r]` 减少了 `k`，因此 `diff[r+1]` 要减去 `k`。

这样，一次区间加法只需要更新两个位置（或一个位置，若 `r+1=n` 则无需更新），从而实现了 O(1) 的修改。
